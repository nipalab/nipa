...
func TestEndToEnd_CloneAddPushFetch(t *testing.T) {
	...
	require.Equal(t, chunker.EncodingZstd, snap1.Files[0].Encoding)
	...
	encoding, chunked, err := chunker.EncodeBytes([]byte(contentA), "a.txt", false, "")
	require.NoError(t, err)
	...
	for h, data := range downloaded {
		...
		decoded, err := chunker.Decode(encoding, data)
		require.NoError(t, err)
...
func TestEndToEnd_SwitchBranch(t *testing.T) {
	...
	defer pinned.Close()
	featureBranch, err := grpcClient.GetBranchByName(ctx, e2eOrgSlug, e2eProjectSlug, "feature")
	require.NoError(t, err)
	require.NotNil(t, featureBranch.CommitID)
	gotPinned, err := pinned.LoadCommit()
	require.NoError(t, err)
	require.Equal(t, featureBranch.CommitID.Base36(), gotPinned.CommitID, "a switch must pin the new branch head")
	require.Empty(t, gotPinned.CommitHash)
